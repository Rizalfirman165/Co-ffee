...
import com.rizalfirman.co_ffee.databinding.ActivityDeteksiBinding
...
class DeteksiActivity : AppCompatActivity() {
    ...
    private val tampilIntentGallery = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK){
            ...
            val myFile = Media.uriToFile(selectedImg, this@DeteksiActivity)
            getFile = myFile
            deteksiViewModel.getFile(myFile)
    ...
    private fun TakePhoto() {
        ...
        Media.createTempFile(application).also {
            val photoUri: Uri = FileProvider.getUriForFile(this@DeteksiActivity, "com.rizalfirman.co_ffee", it)
    ...
    private fun uploadPhoto(){
        if (getFile != null) {
            ...
            deteksiViewModel.predict.observe(this) { result ->
                if (result != null) {
                    when (result) {
                        ...
                        is ConfigResult.Success -> {
                            ...
                            val intent =
                                Intent(this@DeteksiActivity, ResultActivity::class.java).apply {
                                    putExtra(
                                        ResultActivity.EXTRA_DATA,
                                        result.data
                            ...
                            Toast.makeText(this, result.data.diagnose, Toast.LENGTH_SHORT).show()
                            finish()
                        }
    ...
    private fun setupViewModel(){
        val factory : DeteksiViewModelFactory = DeteksiViewModelFactory.getInstance(this)
